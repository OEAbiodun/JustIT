...
logging.basicConfig(filename=r"Week 10\Completed code\Pt8_Exceptions_CollectionsCodeBase\1_Exceptions/file2.log", level=logging.DEBUG) 
...
try:  # attempt to run the indented code block
    ...
    logging.info(f"on {time.asctime()}, Divided {num1} / {num2} = {answer}, from file {fileName}")

except ZeroDivisionError:  # handles exception if code in try block fails
    #Output for user/what the user see
    print("You can't divide a number by zero")
    logging.warning("on {time.asctime()}, User attempted to divide by zero")
